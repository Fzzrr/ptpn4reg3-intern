...
<body>
    ...
    <div class="p-4 sm:ml-64">
                    ...
                    <div class="overflow-hidden ">
                        <table class=" min-w-full rounded-xl">
                            <thead>
                                <tr class="bg-gray-50">
                                    <th scope="col"
                                        class="p-5 text-center text-sm leading-6 font-semibold text-gray-900 capitalize rounded-t-xl">
                                        Username </th>
                                    <th scope="col"
                                        class="p-5 text-center text-sm leading-6 font-semibold text-gray-900 capitalize">
                                        Level </th>
                                    <th scope="col"
                                        class="p-5 text-center text-sm leading-6 font-semibold text-gray-900 capitalize">
                                        Kode Unit </th>
                                    ...
                                    <tr class="bg-white transition-all duration-500 hover:bg-gray-50 ">
                                        ...
                                        <td class=" p-5 items-center">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <form action="/account/{{ $usr->username }}" method="POST"
        ...
        <!-- Overlay background -->
        <div id="modalOverlay"
            class="fixed inset-0 bg-opacity-30 backdrop-blur-sm opacity-0 invisible z-40 transition-opacity duration-300">
        </div>

        <!-- Modal -->
        <div id="authentication-modal" tabindex="-1" aria-hidden="true"
            class="opacity-0 invisible overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 
    justify-center items-center w-full md:inset-0 h-[100vh] max-h-full transition-opacity duration-300 flex">
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <div class="flex items-center justify-between p-4 border-b rounded-t border-gray-200">
                        <h3 id="modalTitle" class="mx-auto text-xl font-semibold text-gray-900 dark:text-white">
                            Tambahkan Akun Pengguna
                        </h3>
                        <button type="button" id="closeModalBtn"
                            class="text-gray-400 bg-transparent cursor-pointer rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center">
                    ...
                    <div class="p-6 space-y-6">
                        <form id="modalForm" method="POST" action="{{ route('account.add') }}" class="space-y-4">
                            @csrf
                            @method('PUT')
        ...
        dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white">
                            ...
                            <div>
                                <button type="submit" id="modalSubmitButton"
                                    class="cursor-pointer w-full text-white bg-blue-700 hover:bg-blue-800 rounded-lg text-sm px-5 py-2.5">
                                    Tambahkan Akun
                                </button>
                            </div>
                        </form>
        ...
        <script>
            ...
            const kodeunitInput = document.getElementById('kodeunit');
            ...
            // Buka modal Add dengan tombol tambah biasa
            if (openModalBtn) {
                openModalBtn.addEventListener('click', () => {
                    modalTitle.textContent = 'Tambahkan Akun Pengguna';
                    modalForm.action = "{{ route('account.add') }}";
                    formMethod.value = "POST";
                    usernameInput.value = '';
                    usernameInput.disabled = false; // enable input username saat tambah
                    passwordInput.value = '';
                    levelInput.value = '';
                    kodeunitInput.value = '';
                    submitButton.textContent = 'Tambahkan Akun';
                    showModal();
                });
            }
            ...
            document.querySelectorAll('.edit-button').forEach(button => {
                button.addEventListener('click', function(e) {
                    ...
                    const kodeunit = this.dataset.kodeunit;

                    modalTitle.textContent = 'Edit Akun Pengguna';
                    modalForm.action = `/account/${username}`; // Pastikan route ini sesuai route update Anda
                    formMethod.value = "PUT";

                    usernameInput.value = username;
                    usernameInput.disabled = true; // disable username saat edit agar tidak diubah
                    passwordInput.value = ''; // Kosongkan password agar pengguna isi hanya saat ingin ganti
                    levelInput.value = level;
                    kodeunitInput.value = kodeunit;

                    submitButton.textContent = 'Update Akun';
